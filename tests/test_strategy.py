...
def test_position_per_grid():
    strategy = GridTradingStrategy(11, 10, 5, 10000, 20000, 100, 100)
    assert strategy.position_per_grid == 2000


def test_position_per_grid_cannot_be_divided():
    with pytest.raises(Exception):
        strategy = GridTradingStrategy(11, 10, 15, 10000, 20000, 100, 100)
...
def test_grids_cannot_be_divided():
    with pytest.raises(Exception):
        strategy = GridTradingStrategy(11, 10, 3, 10000, 20000, 100, 100)


def test_cal_grid_index_by_price():
    strategy = GridTradingStrategy(11, 10, 10, 10000, 20000, 100, 100)
